...
def new_chat():
    cid = str(uuid.uuid4())
    # Start empty - SessionMemory.__init__ fills in the default shape the
    # moment it wraps this dict, so there's only one place (memory.py)
    # that defines what a chat's state looks like.
...
# ---------------- STREAMING ----------------
# Cosmetic only: the reply is already fully generated by this point, this
# just reveals it word-by-word for a typing effect.
...
/* Sidebar button style (ChatGPT-like flat list) */
...
/* Active chat: Streamlit's own type="primary" button, recolored to match
   the theme instead of Streamlit's default red. This is the actual fix
   for highlighting - the old .active-chat class was never applied to
   anything, so it did nothing. */
...
with st.sidebar:
    ...
    for cid in st.session_state["chat_order"]:
        chat_state = st.session_state["chats"][cid]
        title = chat_state.get("title") or "New Chat"
